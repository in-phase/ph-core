...
module Phase::Buffered::Indexed
  class ElemAndCoordIterator(S, E, I)
    ...
    @coord_iter : Indexed::StrideIterator(I)
    @src : S
    
    delegate :reset, :reverse!, to: @coord_iter
    
    private def initialize(@src : S, el_type : E.class, @coord_iter : Indexed::StrideIterator(I))
      shape = @src.shape
    ...
    def self.new(src : Buffered)
      iter = Indexed::LexIterator.cover(src.shape)
      new(src, iter)
    end
    ...
    def clone : self
      Indexed::ElemAndCoordIterator.new(@src, @coord_iter.clone)
    end
  end
