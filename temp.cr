...
module ReadUtils(T)
  ...
  def build(shape, &block)
    coord_iter = LexIterator.cover(shape.to_a)
    buffer = Slice.new(shape.product) do
      yield coord_iter.unsafe_next
    end
...
data = RONArray.new([1, 2], Slice[3, 5])
data = data.tile(5, 3)
puts data
...
res = data.map_with(other) do |d1, d2|
  d1 + d2
end
